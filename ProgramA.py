...
channel.queue_declare(queue='numberStoreB') # Declare queue
channel.queue_declare(queue='pointStoreA') # Declare queue

def success(increase):
    if increase == True:
        print("SUCCESSFUL PREDICT\n")
        channel.basic_publish(exchange='',routing_key='pointStoreA',body="1 Point")
    if increase == False:
        print("unsuccessful\n")
    return
...
def callback(ch, method, properties, body):
    ...
    # Send quest number
    channel.basic_publish(exchange='',routing_key='numberStoreA',body=str(quest))

def listener():
    print('Press CTRL+C to exit to game\nThe game will begin\n')
    ## Listens to programB 
    channel.basic_consume(on_message_callback=callback,queue='numberStoreB')
